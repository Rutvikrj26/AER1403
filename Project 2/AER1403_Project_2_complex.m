...
Y=zeros(nx,ny);
for i = 1:ny
    X(i,:) = linspace(ellipse_points(1,i),right_points(1,i),nx);
    Y(i,:) = linspace(ellipse_points(2,i),right_points(2,i),nx);
end
for i = 1:nx
    X(i+ny-1,:) = linspace(ellipse_points(1,i+ny-1),top_points(1,i),ny);
    Y(i+ny-1,:) = linspace(ellipse_points(2,i+ny-1),top_points(2,i),ny);
end
...
total_nodes = p*q;
bottom_nodes = 1:q;
top_nodes = linspace(ceil(p/2)*q,p*q,q);
...
Sg_elements_vm = zeros(total_elements,4);
Sg_elements_yy = zeros(total_elements,4);
...
for i = 1:total_elements
    ...
    Sg_curr = stress(element_X_coord(i,:),element_Y_coord(i,:),Q);
    Sg_elements_vm(i,:) = transpose(Sg_curr(:,1));
    Sg_elements_yy(i,:) = transpose(Sg_curr(:,2));
    for j = 1:4
        curr_node = all_nodes(i,j);
        Sg_nodes(curr_node,1) = Sg_nodes(curr_node,1) + Sg_elements_vm(i,j); 
        Sg_nodes(curr_node,2) = Sg_nodes(curr_node,2) + Sg_elements_yy(i,j);
...
tot_disp = [tot_disp sum(abs(disp_new))];  
...
%% Functions for Stiffness & Gather Matrix Calculations
    ...
    for i = 1:2
        for j = 1:2
            ...
            H = J\H;
            H = [H(1,1) 0 H(1,2) 0 H(1,3) 0 H(1,4) 0; 0 H(2,1) 0 H(2,2) 0 H(2,3) 0 H(2,4); H(2,1) H(1,1) H(2,2) H(1,2) H(2,3) H(1,3) H(2,4) H(1,4)]; 
            stress_temp(node_num,:) = transpose(D*H*Q); % Sigma XX, Sigma YY, Sigma XY
            node_num = node_num + 1;
        end
    end
