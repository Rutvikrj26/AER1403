...
% Generate Node Coordiantes
    ...
    for i = 1:ny
        ...
        X(i,2) = X(i,1)+(right_points(1,i)-ellipse_points(1,i)) * ((1-gradient)/(1-gradient^(R-1)));
        for j = 3:R
    ...
    for i = 1:nx
        ...
        X(i+ny-1,2) = X(i+ny-1,1)+(top_points(1,i)-ellipse_points(1,i+ny-1)) * ((1-gradient)/(1-gradient^(R-1)));
        for j = 3:R
    ...
    for i = 1:nx
        ...
        Y(i,2) = Y(i,1)+(right_points(2,i)-ellipse_points(2,i)) * ((1-gradient)/(1-gradient^(R-1)));
        for j = 3:R
    ...
    for i = 1:ny
        ...
        Y(i+nx-1,2) = Y(i+nx-1,1)+(top_points(2,i)-ellipse_points(2,i+nx-1)) * ((1-gradient)/(1-gradient^(R-1)));
        for j = 3:R
...
% tot_disp = [tot_disp sum(abs(disp))];  
 ...
 ingle_stress = [ingle_stress sigma*(1 + 2*(a/b))]; 
...
hold on
plot(linspace(0.01,0.2,10),tot_stress)
plot(linspace(0.01,0.2,10),ingle_stress)
title('Varying values of b')
xlabel('value of b in [m]')
ylabel('stress in [Pa]')
hold off
